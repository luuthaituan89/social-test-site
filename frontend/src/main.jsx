...
function Avatar({user,size=42,onClick}){return user?.avatar_url?<img className="avatar clickable" style={{width:size,height:size}} src={asset(user.avatar_url)} alt={user?.name?`${user.name}'s avatar`:"User avatar"} decoding="async" onClick={onClick}/>:<div className="avatar avatar-fallback clickable" style={{width:size,height:size}} aria-label={user?.name?`${user.name}'s avatar`:"User avatar"} onClick={onClick}>{(user?.name||"?")[0]}</div>}
...
function PostComposer({onCreated}){
  ...
  return <form className="card composer" onSubmit={submit}>
    ...
    {selectedGif&&<div className="composer-gif-preview"><img decoding="async" loading="lazy" src={selectedGif.preview_url} alt={selectedGif.title||"Selected GIF"}/><button type="button" onClick={()=>setSelectedGif(null)} aria-label="Remove GIF"><X size={17}/></button><span>GIF</span></div>}
...
function PostCard({post,me,onOpenProfile,onUpdated,onDeleted,onShared,onHidden}){
  ...
  async function react(type){try{const d=await api(`/api/posts/${post.id}/like`,{method:"POST",body:JSON.stringify({reaction:type})});onUpdated?.({...post,...d})}catch(e){notify(e.message)}}
  async function openReactions(){if(!(post.likes_count>0))return;setReactionFilter("all");setReactionDialog({loading:true,items:[]});try{const data=await api(`/api/posts/${post.id}/reactions`);setReactionDialog({loading:false,items:Array.isArray(data.items)?data.items:[]})}catch(e){setReactionDialog(null);notify(e.message)}}
  async function addComment(e){e.preventDefault();if(!comment.trim())return;try{const row=await api(`/api/posts/${post.id}/comments`,{method:"POST",body:JSON.stringify({content:comment})});setComment("");onUpdated?.({...post,comments:[...(post.comments||[]),row]})}catch(e){notify(e.message)}}
  async function saveEdit(){setBusy(true);try{const row=await api(`/api/posts/${post.id}`,{method:"PUT",body:JSON.stringify(draft)});setEditing(false);setMenu(false);onUpdated?.(row)}catch(e){notify(e.message)}finally{setBusy(false)}}
  async function remove(){if(!await confirmDialog({title:"Delete post?",message:"This post and its activity will be permanently removed.",detail:"This action cannot be undone.",confirmLabel:"Delete post"}))return;try{await api(`/api/posts/${post.id}`,{method:"DELETE"});onDeleted?.(post.id)}catch(e){notify(e.message)}}
  ...
  async function hide(kind){try{await api(`/api/posts/${post.id}/${kind}`,{method:"POST"});setMenu(false);onHidden?.(post.id)}catch(e){notify(e.message)}}
  async function toggleFollow(){try{const following=!post.author_following;await api(`/api/users/${post.author.id}/follow`,{method:following?"POST":"DELETE"});setMenu(false);onUpdated?.({...post,author_following:following})}catch(e){notify(e.message)}}
  async function toggleFavorite(){try{const favorite=!post.author_favorite;await api(`/api/posts/authors/${post.author.id}/preference`,{method:"PUT",body:JSON.stringify({favorite})});setMenu(false);onUpdated?.({...post,author_favorite:favorite})}catch(e){notify(e.message)}}
  async function snooze(){try{await api(`/api/posts/authors/${post.author.id}/preference`,{method:"PUT",body:JSON.stringify({snooze_days:30})});setMenu(false);onHidden?.(post.id)}catch(e){notify(e.message)}}
  async function openSave(){setMenu(false);try{let rows=await api("/api/posts/collections");if(!rows.length){const created=await api("/api/posts/collections",{method:"POST",body:JSON.stringify({name:"Saved posts"})});rows=[created]}setSaveDialog({collections:rows,selected:rows[0]?.id,name:"",busy:false})}catch(e){notify(e.message)}}
  ...
  async function unsave(){try{await api(`/api/posts/${post.id}/save`,{method:"DELETE"});setMenu(false);onUpdated?.({...post,is_saved:false})}catch(e){notify(e.message)}}
...
function ProfileAlbums({user,me,onProfileChanged}){
  ...
  async function openAlbum(id){try{setSelecting(false);setSelectedMedia(new Set());setSelected(await api(`/api/albums/${id}`))}catch(e){notify(e.message)}}
  async function create(e){e.preventDefault();setBusy(true);try{const row=await api("/api/albums",{method:"POST",body:JSON.stringify(form)});setCreating(false);setForm({name:"",description:"",privacy:"friends",audience:emptyAudience()});await load();await openAlbum(row.id)}catch(e){notify(e.message)}finally{setBusy(false)}}
  ...
  async function saveAlbum(e){e.preventDefault();setBusy(true);try{await api(`/api/albums/${selected.id}`,{method:"PUT",body:JSON.stringify(editForm)});setEditing(false);await load();await openAlbum(selected.id);await onProfileChanged?.()}catch(e){notify(e.message)}finally{setBusy(false)}}
  async function deleteAlbum(){if(!await confirmDialog({title:"Delete album?",message:`“${selected.name}” will be permanently deleted.`,detail:"All photos, videos and linked timeline posts in this album will also be removed.",confirmLabel:"Delete album"}))return;setBusy(true);try{await api(`/api/albums/${selected.id}`,{method:"DELETE"});setEditing(false);setSelected(null);setPreview(null);await load();await onProfileChanged?.()}catch(e){notify(e.message)}finally{setBusy(false)}}
  async function upload(files){
    if(!selected||!files?.length)return;setBusy(true);
    try{for(const file of files){const fd=new FormData();fd.append("file",file);fd.append("caption","");await api(`/api/albums/${selected.id}/media`,{method:"POST",body:fd})}await openAlbum(selected.id);await load();await onProfileChanged?.()}catch(e){notify(e.message)}finally{setBusy(false)}
  }
  ...
  async function deleteSelected(){
    ...
    setBusy(true);
    try{for(const id of selectedMedia)await api(`/api/albums/${selected.id}/media/${id}`,{method:"DELETE"});setSelecting(false);setSelectedMedia(new Set());await openAlbum(selected.id);await load();await onProfileChanged?.()}catch(e){notify(e.message)}finally{setBusy(false)}
  }
  return <section id={`profile-albums-${user.id}`} className="card profile-albums-section"><div className="albums-head"><div><h2>Albums</h2><p className="muted">Profile pictures, cover photos and timeline media</p></div>{mine&&<button className="primary" onClick={()=>setCreating(true)}>Create album</button>}</div>
    <div className="album-grid">{albums.map(a=><button className="album-card" key={a.id} onClick={()=>openAlbum(a.id)}><div className="album-cover">{a.cover?(a.cover_type==="video"?<video src={asset(a.cover)} muted preload="metadata"/>:<img decoding="async" loading="lazy" src={asset(a.cover)} alt={a.name}/>):<ImageIcon size={38}/>}</div><b>{a.name}</b>{a.description&&<small>{a.description}</small>}<span>{a.media_count} item{a.media_count===1?"":"s"} · {a.privacy}</span></button>)}</div>
...
function Profile({data,me,reload,message,open}){
  ...
  async function deletePhoto(){
    ...
    try{await api(`/api/users/me/${kind}`,{method:"DELETE"});setPhotoMenu(null);await reload(user.id)}catch(e){notify(e.message)}
  }
  ...
  async function toggleFollow(){
    try{await api(`/api/users/${user.id}/follow`,{method:data.is_following?"DELETE":"POST"});await reload(user.id)}catch(e){notify(e.message)}
  }
  ...
  return <>
    ...
    {photoMenu&&<div className="modal-backdrop" onClick={()=>setPhotoMenu(null)}><div className="photo-action-modal" onClick={e=>e.stopPropagation()}>
      <img decoding="async" loading="lazy" src={asset(photoMenu==="avatar"?user.avatar_url:user.cover_url)} className={photoMenu==="avatar"?"photo-action-avatar":"photo-action-cover"} alt={photoMenu==="avatar"?"Profile picture":"Cover photo"}/>
...
function GroupChatInfo({group,friends,onClose,onChanged,onApplied,onLeave}){
  ...
  async function avatar(file){if(!file)return;const fd=new FormData();fd.append("file",file);try{const up=await api("/api/chat/upload",{method:"POST",body:fd});setDetails(v=>({...v,avatar_url:up.url}))}catch(e){notify(e.message)}}
  async function memberAction(member,action){try{if(action==="remove")await api(`/api/chat/group-conversations/${group.group_chat_id}/members/${member.id}`,{method:"DELETE"});else if(action==="add")await api(`/api/chat/group-conversations/${group.group_chat_id}/members/${member.id}`,{method:"POST"});else await api(`/api/chat/group-conversations/${group.group_chat_id}/members/${member.id}/role?role=${action}`,{method:"PUT"});await reload()}catch(e){notify(e.message)}}
  async function nickname(member){const value=await promptDialog({title:`Nickname for ${member.name}`,label:"Nickname",defaultValue:member.nickname||"",confirmLabel:"Save nickname"});if(value===null)return;try{await api(`/api/chat/group-conversations/${group.group_chat_id}/members/${member.id}/nickname?nickname=${encodeURIComponent(value)}`,{method:"PUT"});await reload();notify("Nickname updated",{type:"success"})}catch(e){notify(e.message)}}
  async function preferences(minutes,sound=details.notification_sound||"default"){try{await api(`/api/chat/group-conversations/${group.group_chat_id}/preferences?mute_minutes=${minutes}&sound=${encodeURIComponent(sound)}`,{method:"PUT"});await reload()}catch(e){notify(e.message)}}
  async function reviewRequest(request,action){try{await api(`/api/chat/group-conversations/${group.group_chat_id}/join-requests/${request.id}/${action}`,{method:"POST"});await reload()}catch(e){notify(e.message)}}
...
function DirectChatInfo({target,me,messages,onClose,onApplied,openProfile}){
  ...
  useEffect(()=>{api(`/api/chat/${target.id}/settings`).then(setDetails).catch(e=>notify(e.message))},[target.id]);
  ...
  async function save(){setBusy(true);try{const result=await api(`/api/chat/${target.id}/settings`,{method:"PUT",body:JSON.stringify({...details,mute_minutes:Number(details.mute_minutes||0)})});onApplied(result);onClose()}catch(e){notify(e.message)}finally{setBusy(false)}}
  ...
  async function block(){if(!await confirmDialog({variant:"danger",title:`Block ${target.name}?`,message:"You will no longer be able to message each other.",confirmLabel:"Block"}))return;try{await api(`/api/users/${target.id}/block`,{method:"POST"});onClose()}catch(e){notify(e.message)}}
  return <div className="modal-backdrop group-info-backdrop" onMouseDown={onClose}><aside className="group-chat-info" onMouseDown={e=>e.stopPropagation()}><div className="group-info-head"><b>Conversation settings</b><button onClick={onClose}><X/></button></div><div className="group-info-scroll">
    ...
    <section><h3>Media & files</h3><div className="group-info-media">{media.map(item=><a key={item.id} href={asset(item.attachment_url)} target="_blank" rel="noreferrer">{["image","gif"].includes(item.message_type)?<img decoding="async" loading="lazy" src={asset(item.attachment_url)}/>:<><FileText/><small>{item.attachment_name||item.message_type}</small></>}</a>)}{!media.length&&<p className="muted">No shared media or files.</p>}</div></section>
...
function MediaPreview({media,onClose}){
  ...
  return <div className="media-preview-backdrop" onClick={onClose}><div className="media-preview-modal" onClick={e=>e.stopPropagation()}>
    ...
    <div className="media-preview-body">{media.type==="video"?<video src={media.url} controls autoPlay playsInline/>:<img decoding="async" loading="lazy" src={media.url} alt={media.name||"Image preview"}/>}</div>
  </div></div>
}
...
function Chat({me,target,open,onChanged,onSent,onRequestAction,onGroupActivated,onGroupSettingsChanged,onMobileBack,friends=[]}){
  ...
  const[historyLoading,setHistoryLoading]=useState(false),[visibleMessageCount,setVisibleMessageCount]=useState(120);
  ...
  useEffect(()=>{
    setMessages([]);setVisibleMessageCount(120);setSelectedStickers([]);setShowStickers(false);setSelectedGif(null);setShowGifPicker(false);setGifQuery("");setReplyingTo(null);setMessageMenu(null);setForwarding(null);setIsOtherTyping(false);setPresence({online:false,last_seen_at:null,active_status_visible:false});
  ...
  async function send(){
    ...
    try{
      ...
      notify(`Could not send message: ${e.message}`);
    }finally{
      setSending(false);
    }
  }
  ...
  async function uploadAttachment(file,forcedType){
    ...
    }catch(e){notify(`Could not send attachment: ${e.message}`)}finally{setSending(false)}
  }
  ...
  async function toggleMessagePin(message){
    ...
    catch(e){notify(e.message)}finally{setActionBusy(false)}
  }
  ...
  async function openForward(message){
    ...
    try{const rows=await api("/api/chat/conversations");setForwardTargets((Array.isArray(rows)?rows:[]).filter(item=>item?.user?.id))}catch(e){notify(e.message);setForwarding(null)}
  }
  ...
  async function toggleRecording(){
    ...
    }catch(e){notify("Microphone access is required to record voice messages.")}
  }

  async function createPoll(){
    if(!target?.is_group||target.is_group_draft||!pollQuestion.trim()||pollOptions.filter(x=>x.trim()).length<2)return;
    try{const result=await api(`/api/chat/group-conversations/${target.group_chat_id}/polls`,{method:"POST",body:JSON.stringify({question:pollQuestion,options:pollOptions})});mergeMessages(result);setPollOpen(false);setPollQuestion("");setPollOptions(["",""])}catch(e){notify(e.message)}
  }
  async function votePoll(pollId,optionId){try{const result=await api(`/api/chat/polls/${pollId}/vote/${optionId}`,{method:"POST"});mergeMessages(result)}catch(e){notify(e.message)}}
  async function sendQuickReaction(){try{const endpoint=target.is_group?`/api/chat/group-conversations/${target.group_chat_id}/messages`:`/api/chat/${target.id}/messages`,reaction=(target.is_group?target.quick_reaction:directSettings?.quick_reaction)||"👍";const result=await api(endpoint,{method:"POST",body:JSON.stringify({content:"",message_type:"sticker",sticker:JSON.stringify([reaction])})});mergeMessages(result);onChanged?.();onSent?.()}catch(e){notify(e.message)}}
  ...
  return <div className={`chat card chat-theme-${target.is_group?(target.theme||"default"):(directSettings?.theme||"default")}`}>
    ...
    <div className="messages" onScroll={()=>setReactionMenu(null)}>
      {historyLoading&&<LoadingSkeleton rows={5} label="Loading message history"/>}
      {!historyLoading&&messages.length>visibleMessages.length&&<div className="history-window-notice"><button type="button" className="secondary" onClick={()=>setVisibleMessageCount(count=>count+120)}>Show earlier messages</button></div>}
      {visibleMessages.map(x=><div key={x.id} className={`bubble ${x.sender_id===Number(me.id)?"mine":""} attachment-bubble ${x.message_type==="text"&&isEmojiOnlyMessage(x.content)?"emoji-only-bubble":""}`}>
        ...
        {x.message_type==="image"&&x.attachment_url&&<button className="chat-media-button" onClick={()=>setPreviewMedia({type:"image",url:asset(x.attachment_url),name:x.attachment_name})}><img decoding="async" loading="lazy" className="chat-image" src={asset(x.attachment_url)} alt={x.attachment_name||"Image"} onLoad={()=>scrollChatToEnd("auto")}/><span className="media-hover-icon"><Maximize2 size={20}/></span></button>}
        ...
        {x.message_type==="gif"&&x.attachment_url&&<button className="chat-media-button chat-gif-button" onClick={()=>setPreviewMedia({type:"image",url:asset(x.attachment_url),name:x.attachment_name})}><img decoding="async" loading="lazy" className="chat-gif" src={asset(x.attachment_url)} alt={x.attachment_name||"GIF"} onLoad={()=>scrollChatToEnd("auto")}/><span className="gif-label">GIF</span></button>}
    ...
    <div className="chat-input">
      ...
      <div className={`chat-compose-field ${selectedStickers.length||selectedGif?"has-sticker":""}`}>
        ...
        {selectedGif&&<div className="selected-gif" title={selectedGif.title}><img decoding="async" loading="lazy" src={selectedGif.preview_url} alt={selectedGif.title||"Selected GIF"}/><button type="button" onClick={()=>setSelectedGif(null)} aria-label="Remove GIF"><X size={12}/></button><span>GIF</span></div>}
...
function SettingsPage({user,onUserUpdated,onAccountClosed,openProfile,openMessage,theme="dark",onThemeChange,language="en",onLanguageChange,timezone="auto",onTimezoneChange,notificationPrefs=MESSAGE_NOTIFICATION_DEFAULTS,onNotificationPrefsChange}){
  ...
  const[webPush,setWebPush]=useState({supported:false,configured:false,subscribed:false}),[webPushBusy,setWebPushBusy]=useState(false);
  ...
  useEffect(()=>{pushStatus().then(setWebPush).catch(()=>{})},[user.id]);
  ...
  async function toggleWebPush(){
    setWebPushBusy(true);setPrivacyError("");
    try{setWebPush(webPush.subscribed?await disablePush():await enablePush())}
    catch(e){setPrivacyError(e.message)}finally{setWebPushBusy(false)}
  }
  ...
  return <div className="card settings-page">
    ...
    <div className="settings-section appearance-settings">
      <h3>{t("Appearance")}</h3>
      <p className="muted small">{t("Choose how SocialN looks on this browser.")}</p>
      <div className="theme-options" role="radiogroup" aria-label={t("Color theme")}>
        <button type="button" role="radio" aria-checked={theme==="light"} className={`theme-option ${theme==="light"?"selected":""}`} onClick={()=>changeTheme("light")}><Sun size={22}/><span><b>{t("Light")}</b><small>{t("Bright background")}</small></span><i>{theme==="light"?"✓":""}</i></button>
        <button type="button" role="radio" aria-checked={theme==="dark"} className={`theme-option ${theme==="dark"?"selected":""}`} onClick={()=>changeTheme("dark")}><Moon size={22}/><span><b>{t("Dark")}</b><small>{t("Easy on the eyes")}</small></span><i>{theme==="dark"?"✓":""}</i></button>
      </div>
    </div>
    <div className="settings-section language-settings">
      <h3>{t("Language")}</h3>
      <p className="muted small">{t("Choose the language used across SocialN.")}</p>
    ...
    <div className="settings-section message-notification-settings">
      ...
      <label className="settings-switch-row"><span><b>Background Web Push</b><small>Receive notifications through a Service Worker even when no SocialN tab is open.</small></span><input type="checkbox" role="switch" checked={Boolean(webPush.subscribed)} disabled={webPushBusy||!webPush.supported||!webPush.configured} onChange={toggleWebPush}/></label>
      {webPush.supported&&!webPush.configured&&<div className="notification-permission">Web Push requires VAPID keys in the server environment.</div>}
    ...
    <div className="settings-section">
      ...
      <form onSubmit={submitUsername} autoComplete="off">
        <label htmlFor="settings-username">Username<input id="settings-username" name="settings-username" value={username} disabled={usernameStatus&&!usernameStatus.can_change} onChange={e=>setUsername(e.target.value)} autoComplete="off" autoCapitalize="none" spellCheck={false}/></label>
    ...
    <div className="settings-section">
      ...
      <form onSubmit={submit}>
        <label htmlFor="settings-current-password">Current password<input id="settings-current-password" name="current-password" type="password" autoComplete="current-password" value={f.current_password} onChange={e=>setF({...f,current_password:e.target.value})}/></label>
        <label htmlFor="settings-new-password">New password<input id="settings-new-password" name="new-password" type="password" autoComplete="new-password" value={f.new_password} onChange={e=>setF({...f,new_password:e.target.value})}/></label>
        <label htmlFor="settings-confirm-password">Confirm new password<input id="settings-confirm-password" name="confirm-password" type="password" autoComplete="new-password" value={f.confirm_password} onChange={e=>setF({...f,confirm_password:e.target.value})}/></label>
...
<div className="group-hero-cover">{group.cover_url?<img decoding="async" loading="lazy" src={asset(group.cover_url)}/>:<Users size={70}/>} {group.my_role==="admin"&&<label className={`group-cover-change ${coverBusy?"busy":""}`}><Camera size={18}/>{coverBusy?"Uploading...":"Change cover photo"}<input hidden type="file" accept="image/*" disabled={coverBusy} onChange={e=>{changeCover(e.target.files?.[0]);e.target.value=""}}/></label>}</div>
...
function RoutedApp(){
  ...
  function setRoute(path,nextView,{replace=false}={}){
    ...
    clearTimeout(searchTimerRef.current);searchRequestRef.current+=1;setSearch("");setResults([]);
  ...
  function query(v){
    ...
    setSearch(v);clearTimeout(searchTimerRef.current);
    if(!v.trim()){setResults([]);return}
    searchTimerRef.current=setTimeout(async()=>{try{const data=await api(`/api/search?q=${encodeURIComponent(v.trim())}`),order=["people","posts","groups","pages","reels","events","marketplace"];if(requestId!==searchRequestRef.current)return;setResults(order.flatMap(kind=>(data.results?.[kind]||[]).map(item=>({...item,_kind:kind}))));}catch{if(requestId===searchRequestRef.current)setResults([])}},350)
  }
  ...
  async function respondRelationship(n,action){try{await api(`/api/users/relationship-requests/${n.entity_id}/${action}`,{method:"POST"});await loadN();const updated=await api("/api/users/me");setUser(updated);if(view==="profile"&&Number(profile?.user?.id)===Number(user.id))await reloadProfile(user.id)}catch(e){notify(e.message)}}
  ...
  return <div><ConfirmHost/><MetroMessageToasts items={[...messageToasts,...activityToasts].slice(0,4)} onOpen={item=>item.kind==="activity"?openActivityNotification(item):openMessageNotification(item)} onDismiss={id=>{setMessageToasts(rows=>rows.filter(row=>row.toastId!==id));setActivityToasts(rows=>rows.filter(row=>row.toastId!==id))}}/>
    <header className="topbar"><button className="sidebar-toggle" onClick={toggleSidebar} aria-label={mobileSidebarOpen||!sidebarCollapsed?"Close navigation menu":"Open navigation menu"} aria-expanded={mobileSidebarOpen||!sidebarCollapsed}><Menu size={23}/></button><div className="brand" onClick={()=>go("/","home")}>Social<span>N</span></div><div className="search" role="search"><Search/><input type="search" name="socialn-global-search" autoComplete="off" autoCapitalize="none" spellCheck={false} aria-label="Search SocialN" data-1p-ignore="true" data-lpignore="true" value={search} onChange={e=>query(e.target.value)} placeholder={t("Search SocialN...")}/>{results.length>0&&<div className="search-results global-search-results">{results.map((item,index)=><button key={`${item._kind}-${item.id}-${index}`} onClick={()=>openSearchResult(item)}>{item._kind==="people"?<Avatar user={item}/>:<span className="search-kind">{item._kind[0].toUpperCase()}</span>}<span>{item.name||item.title||item.caption||item.content}<small>{item._kind}{item.username?` · @${item.username}`:""}</small></span></button>)}</div>}</div>
      <div className="top-actions">
        <button className="message-icon-btn top-messages" title={t("Messages")} onClick={()=>{setTarget(null);go("/messages","chat")}}><MessageCircle/>{messageUnread>0&&<span className="badge">{messageUnread>99?"99+":messageUnread}</span>}</button>
        <div className="notif-wrap top-notifications"><button className="notification-btn" title={t("Notifications")} onClick={()=>{setDrop(value=>!value);setAccountDrop(false)}}><Bell/>{unread>0&&<span className="badge">{unread}</span>}</button>{drop&&<div className="notif-dropdown"><div className="notif-dropdown-head"><b>{t("Notifications")}</b><button onClick={async()=>{await api("/api/notifications/read-all",{method:"POST"});loadN()}}>{t("Mark all read")}</button></div><div className="notif-scroll">{notifs.slice(0,20).map(n=>n.type==="relationship_request"?<div className={`notification-row relationship-notification ${n.is_read?"":"unread"}`} key={n.id}><Avatar user={n.actor}/><div className="notification-content">{n.message}<small>{formatDateTime(n.created_at)}</small><div className="relationship-notification-actions"><button className="primary" onClick={()=>respondRelationship(n,"accept")}>Accept</button><button className="secondary" onClick={()=>respondRelationship(n,"decline")}>Decline</button></div></div></div>:<button className={`notification-row ${n.is_read?"":"unread"}`} key={n.id} onClick={()=>read(n)}><Avatar user={n.actor}/><div className="notification-content">{n.message}<small>{formatDateTime(n.created_at)}</small></div></button>)}</div></div>}</div>
    ...
    <div className={`layout ${sidebarCollapsed?"sidebar-collapsed":""}`}>{mobileSidebarOpen&&<button className="sidebar-backdrop" aria-label="Close navigation menu" onClick={()=>setMobileSidebarOpen(false)}/>}<aside className={`sidebar ${mobileSidebarOpen?"open":""}`}><button className="side-user" title={user.name} onClick={()=>openProfile(user.id)}><Avatar user={user}/><b className="sidebar-label">{user.name}</b></button><button className={view==="home"?"selected":""} title="Home" onClick={()=>go("/","home")}><Home/><span className="sidebar-label">{t("Home")}</span></button><button className={view==="friends"?"selected":""} title="Friends" onClick={()=>go("/friends","friends")}><UserCheck/><span className="sidebar-label">{t("Friends")}</span>{requests.length>0&&<span className="side-badge">{requests.length}</span>}</button><button className={view==="groups"?"selected":""} title="Groups" onClick={()=>{setGroupId(null);go("/groups","groups")}}><UsersRound/><span className="sidebar-label">{t("Groups")}</span></button><button className={view==="chat"?"selected":""} title={t("Messages")} onClick={()=>{setTarget(null);go("/messages","chat")}}><MessageCircle/><span className="sidebar-label">{t("Messages")}</span></button><button className={view==="activity"?"selected":""} title="Activity log" onClick={()=>go("/activity-log","activity")}><History/><span className="sidebar-label">{t("Activity log")}</span></button><button className={view==="settings"?"selected":""} title="Settings" onClick={()=>go("/settings","settings")}><Settings/><span className="sidebar-label">{t("Settings")}</span></button></aside>
      <main className={`main ${view==="chat"?"chat-main":view==="profile"?"profile-main":["home","friends","settings","activity","notifications","groups"].includes(view)?"adaptive-main":""}`}>
        ...
        {view==="home"&&<FeedPage composer={<PostComposer onCreated={p=>setFeed(v=>[p,...v])}/>} posts={feed.map(p=><PostCard key={p.id} post={p} me={user} onOpenProfile={openProfile} onUpdated={updated=>setFeed(v=>v.map(x=>x.id===updated.id?updated:x))} onDeleted={id=>setFeed(v=>v.filter(x=>x.id!==id))} onHidden={id=>setFeed(v=>v.filter(x=>x.id!==id))} onShared={shared=>setFeed(v=>[shared,...v])}/>)} empty={feedLoading?<div className="card"><LoadingSkeleton rows={5} label="Loading feed"/></div>:<div className="card empty">{t("No posts yet. Create the first post.")}</div>} suggestedGroups={feedMeta.groups} onOpenGroup={id=>{setGroupId(id);go(`/groups/${id}`,"groups")}} footer={<div ref={feedSentinelRef} className="feed-load-more">{feedLoading?"Loading more posts…":feedMeta.hasMore?<button className="secondary" onClick={()=>loadFeed({reset:false})}>Load more</button>:feed.length?"You're all caught up":""}</div>}
        />}
        ...
        {view==="not_found"&&<div className="card empty"><h2>{t("Page not found")}</h2><button className="primary" onClick={()=>go("/","home")}>{t("Go home")}</button></div>}
      </main>
    </div>
    <nav className="mobile-bottom-nav" aria-label="Primary navigation">
      <button className={view==="home"?"active":""} onClick={()=>go("/","home")}><Home/><span>{t("Home")}</span></button>
      <button className={view==="friends"?"active":""} onClick={()=>go("/friends","friends")}><UserCheck/>{requests.length>0&&<i>{requests.length>99?"99+":requests.length}</i>}<span>{t("Friends")}</span></button>
      <button className={view==="groups"?"active":""} onClick={()=>{setGroupId(null);go("/groups","groups")}}><UsersRound/><span>{t("Groups")}</span></button>
      <button className={view==="chat"?"active":""} onClick={()=>{setTarget(null);go("/messages","chat")}}><MessageCircle/>{messageUnread>0&&<i>{messageUnread>99?"99+":messageUnread}</i>}<span>{t("Messages")}</span></button>
      <button onClick={()=>setMobileSidebarOpen(value=>!value)} aria-expanded={mobileSidebarOpen}><Menu/><span>{t("Menu")}</span></button>
    </nav>
  </div>
}

createRoot(document.getElementById("root")).render(<ErrorBoundary><I18nProvider><QueryClientProvider client={queryClient}><BrowserRouter><RoutedApp/></BrowserRouter></QueryClientProvider><FeedbackHost/></I18nProvider></ErrorBoundary>);
